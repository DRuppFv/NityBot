...
async fn lang(ctx: &Context, msg: &Message) -> CommandResult {
    ...
    if new_language == serv_lang {
        msg.reply_ping(&ctx.http, "❕ | That is already the current language!")
            .await?;
    } else {
        ...
        if there_is {
            let language_str = match new_language {
                ...
                "pt" => "The current language is 🇧🇷 | Portuguese!",
                "es" => "The current language is 🇪🇸 | Spanish!",
                "de" => "The current language is 🇩🇪 | Deutsch",
                "it" => "The current language is 🇮🇹 | Italian",
                "fr" => "The current language is 🇫🇷 | French",
                "ru" => "The current language is 🇷🇺 | Russian",
                "tr" => "The current language is 🇹🇷 | Turkish",
                "nl" => "The current language is 🇳🇱 | Dutch",
                _ => "",
            };
            ...
            match try_serv_lang {
                Ok(_) => {
                    sqlx::query!(
                        "DELETE FROM serverlang WHERE servid = ?;
